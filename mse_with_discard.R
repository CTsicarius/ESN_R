...
number_of_train <- c(1000, 2500, 5000, 7500, 10000, 25000, 50000)
...
for (sim in 1:N_sim){
  ...
  for (i in 1:length(number_of_train)){
    ...
    #CODE FOR ESN ERROR
    Wout <- train_Wout(y = train_data[, (n_discarded+1):number_of_train[i]],
                       u = train_data[, n_discarded:(number_of_train[i] - 1)],
                       W = W, Win = Win, x = train_x[, n_discarded:(number_of_train[i] - 1)],
